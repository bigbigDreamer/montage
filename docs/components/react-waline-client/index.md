...
| 参数            | 说明                                 | 类型                        | 默认值                        | 版本  |
|---------------|------------------------------------|---------------------------|----------------------------|-----|
| **serverURL** | `waline` 服务地址                      | `string`                  | `-`                        |
| **path**      | 页面路由`path`                         | `string`                  | `window.location.pathname` | 
| **className** | `waline` `container` 的对话类名         | `string`                  | `-`                        | 
| **style**     | 可直接设置 `waline` `container` 的*内联样式* | `CSSProperties`           | `-`                        | 
| **ref**       | 可用于组件外部容器，对话更新组件内部配置(*不需要刷新组件*)    | [`CRefProps`](#CRefProps) | `-`                        | 
...
1. 为什么更新内部配置会通过 `ref` 暴露，而不是内部更新呢？

> 答：因为笔者认为，组件的设计应该尽可能的纯粹，更新哪些配置交给外部去主动更新，开一个**菊花**给外部用，会尽可能的灵活，减少外部的 `state` 声明。
